...
export default function AdminCompletedHistory() {
  ...
  const filteredServices = useMemo(() => {
    return services.filter((item) => {
      ...
      const matchesSearch =
        ...
        (item.vehicleNumber || item.vehicle_number || "")
          .toLowerCase()
          .includes(text) ||
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      ...
      />
      <View className="p-6">
        <View className="flex-end items-end mb-6">
          <View className="bg-card-light/90 px-4 py-2 rounded-2xl border border-slate-700">
        ...
        <View className="flex-row gap-3 mb-6">
          <View className="flex-1 relative">
            ...
            <TextInput
              ...
              className="bg-card border border-slate-700 rounded-2xl pl-12 pr-4 py-3.5 text-white font-bold text-sm"
            />
          </View>
          <TouchableOpacity
            onPress={() => {
            ...
            className="bg-card border border-slate-700 px-4 py-3 rounded-2xl items-center justify-center flex-row gap-2"
          >
          ...
          <FlatList
            ...
            ListEmptyComponent={() => (
              <View className="py-20 items-center justify-center">
                ...
                />
                <Text className="text-text-secondary text-sm font-bold mt-4">
      ...
      >
        <View className="flex-1 bg-black/80 justify-end">
          <View className="bg-background rounded-t-[3rem] border-t border-slate-700 max-h-[90%] pb-10">
            {/* Modal Header */}
            <View className="p-8 pb-4">
              <View className="flex-row justify-between items-center mb-6">
                <View className="flex-row items-center gap-4">
                  ...
                  <View>
                    ...
                    <Text className="text-slate-400 text-[8px] font-black uppercase tracking-widest">
                      Finalized Record Entry
                    </Text>
                  </View>
                </View>
                <TouchableOpacity
              ...
              {selectedServiceDetail && (
                <View className="bg-card-light rounded-3xl p-5 border border-slate-700/50 flex-row justify-between items-center">
                  <View>
            ...
            <View className="flex-row px-8 mt-4">
              <TouchableOpacity
                ...
                className={`flex-1 py-3 items-center border-b-2 ${activeTab === "issues" ? "border-primary" : "border-slate-700"}`}
              >
                <Text
                  className={`text-[10px] font-black uppercase tracking-widest ${activeTab === "issues" ? "text-primary" : "text-slate-500"}`}
                >
                  Diagnostic
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                ...
                className={`flex-1 py-3 items-center border-b-2 ${activeTab === "parts" ? "border-primary" : "border-slate-700"}`}
              >
                <Text
                  className={`text-[10px] font-black uppercase tracking-widest ${activeTab === "parts" ? "text-primary" : "text-slate-500"}`}
                >
                  Spare Parts
                </Text>
              </TouchableOpacity>
            ...
            >
              {activeTab === "issues" ? (
                <View className="gap-3">
                  {(selectedServiceDetail?.issues || []).map(
                    (iss: any, i: number) => (
                      <View
                        key={i}
                        className="bg-card-light/60 p-4 rounded-2xl border border-slate-700 flex-row justify-between items-center"
                      >
                        <View className="flex-1 pr-3">
                          <Text className="text-white font-bold text-sm uppercase">
                            {iss.issue}
                          </Text>
                          <Text className="text-primary/70 text-[8px] font-black uppercase tracking-widest mt-1">
                            Verified Diagnostic
                          </Text>
                        </View>
                ...
                <View className="gap-3">
                  {(selectedServiceDetail?.parts || []).map(
                    (p: any, i: number) => (
                      <View
                        key={i}
                        className="bg-card-light/60 p-4 rounded-2xl border border-slate-700 flex-row justify-between items-center"
                      >
                        <View className="flex-1">
                          ...
                          <Text className="text-text-secondary text-[8px] font-black uppercase tracking-widest mt-1">
                            Qty: {p.qty}
                          </Text>
                        </View>
                        <Text className="text-primary font-black">
            ...
            <View className="px-8 mt-6">
              <TouchableOpacity
                onPress={() => setDetailModalVisible(false)}
                className="w-full py-5 bg-primary rounded-3xl items-center shadow-lg shadow-primary/20"
              >
