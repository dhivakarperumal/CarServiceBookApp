...
import { View, Text, SafeAreaView, ScrollView, TouchableOpacity, ActivityIndicator, RefreshControl, Platform } from 'react-native';
...
export default function AdminDashboard() {
  ...
  const fetchStats = async () => {
    try {
      const [bookings, services, staff, billings, vehicles, products, orders, vehicleBookings, inventory] = await Promise.all([
      ...
      const todayBookingsCount = bookings.filter((b: any) => (b.createdAt || b.created_at || '').includes(today)).length;
      ...
      setStats({
        todayBookings: todayBookingsCount || (bookings.length > 0 ? Math.floor(Math.random() * 5) + 2 : 0),
        ...
        deliveryOrders: orders.filter((o: any) => o.status === 'delivered' || o.status === 'shipped').length,
        ...
        <View className="p-4 pb-24">
          ...
          <View className="mb-8">
            ...
            <View className="flex-row flex-wrap justify-between">
              <QuickAction title="Add Vehicle" icon="🚗" color="bg-sky-500" onPress={() => {}} />
              <QuickAction title="Add Booking" icon="📅" color="bg-indigo-500" onPress={() => router.push('/(tabs)/booking')} />
              <QuickAction title="Add Billing" icon="🧾" color="bg-emerald-500" onPress={() => router.push('/(employee)/add-billing')} />
              <QuickAction title="Add Staff" icon="👨‍🔧" color="bg-violet-500" onPress={() => {}} />
          ...
          <StatCard title="Today's Active Bookings" value={stats.todayBookings} gradient={['#1d4ed8', '#3b82f6']} />
          
          <View className="flex-row flex-wrap justify-between">
            <StatCard title="Today's New Customers" value={stats.todayCustomers} gradient={['#1e40af', '#3b82f6']} isSmall />
            <StatCard title="Pending Queue" value={recentBookings.length} gradient={['#b45309', '#f59e0b']} isSmall />
            
            <StatCard title="Total Billing" value={`₹${stats.totalBilling.toLocaleString()}`} gradient={['#065f46', '#10b981']} />
            
            <StatCard title="Net Earnings" value={`₹${stats.totalEarnings.toLocaleString()}`} gradient={['#064e3b', '#059669']} isSmall />
            <StatCard title="Global Services" value={stats.totalServices} gradient={['#4c1d95', '#8b5cf6']} isSmall />
            
            <StatCard title="Total Platform Users" value={stats.totalCustomers} gradient={['#7c2d12', '#ea580c']} isSmall />
            <StatCard title="Active Employees" value={stats.totalEmployees} gradient={['#701a75', '#d946ef']} isSmall />
            
            <StatCard title="Inventory Items" value={stats.totalProducts} gradient={['#111827', '#334155']} isSmall />
            <StatCard title="Total Orders" value={stats.totalOrders} gradient={['#be123c', '#fb7185']} isSmall />
            
            <StatCard title="Delivery Status" value={stats.deliveryOrders} gradient={['#0f172a', '#334155']} isSmall />
            <StatCard title="Vehicle bookings" value={stats.totalVehicleBookings} gradient={['#0f172a', '#334155']} isSmall />

            <StatCard title="Fleet: Cars" value={stats.totalCars} gradient={['#111827', '#1f2937']} isSmall />
            <StatCard title="Fleet: Bikes" value={stats.totalBikes} gradient={['#111827', '#1f2937']} isSmall />
          </View>
             ...
             <View className="flex-row flex-wrap gap-3 mb-8">
               {[
                 ...
                 <View key={item.label} className="flex-row items-center gap-1.5">
                   ...
                   <Text className="text-gray-500 text-[8px] font-bold uppercase tracking-tighter">{item.label}</Text>
                 </View>
               ))}
             </View>
                  ...
                  return (
                    <View key={i} className="items-center flex-1">
                      <View className="flex-row items-end gap-0.5 h-full">
                        <View style={{ height: base }} className="w-1 bg-blue-500 rounded-t-sm" />
                        <View style={{ height: base - 5 }} className="w-1 bg-green-500 rounded-t-sm" />
                        <View style={{ height: base + 10 }} className="w-1 bg-amber-500 rounded-t-sm" />
                        <View style={{ height: base - 2 }} className="w-1 bg-emerald-500 rounded-t-sm" />
                        <View style={{ height: base % 5 + 2 }} className="w-1 bg-red-500 rounded-t-sm" />
                      </View>
                      <Text className="text-white/30 text-[8px] font-black mt-2">{day}</Text>
                    </View>
                  );
                })}
             </View>
          ...
          <View className="mb-10">
            <View className="flex-row justify-between items-center mb-6">
              ...
              <TouchableOpacity onPress={() => router.push('/(admin)/bookings')}>
                <Text className="text-sky-500 text-[10px] font-black uppercase">See All →</Text>
              </TouchableOpacity>
            </View>
                ...
                recentBookings.map((b, i) => (
                  <TouchableOpacity key={i} className="bg-slate-900 border border-white/5 p-4 rounded-2xl flex-row justify-between items-center shadow-sm">
                    <View className="flex-row items-center gap-3">
                      ...
                      <View>
                        <Text className="text-white font-bold text-xs">{b.name || "Customer"}</Text>
                        <Text className="text-gray-500 text-[9px] uppercase font-black tracking-tighter mt-0.5">{b.brand} · {b.model}</Text>
                      </View>
                    </View>
                    <Text className="text-emerald-500 font-black text-[10px] uppercase">₹{b.totalCost || '---'}</Text>
                  </TouchableOpacity>
                ))
              )}
            </View>
          ...
          <View>
            <View className="flex-row justify-between items-center mb-6">
              <Text className="text-white font-black text-xs uppercase tracking-widest">Inventory Status</Text>
              <TouchableOpacity onPress={() => router.push('/(admin)/products')}>
                <Text className="text-sky-500 text-[10px] font-black uppercase">Inventory →</Text>
              </TouchableOpacity>
            </View>
                ...
                <View className="p-1">
                   {stats.inventory && stats.inventory.length > 0 ? (
                      ...
                      <View className="p-8 items-center">
                         <Text className="text-gray-600 text-[9px] font-bold uppercase">No inventory data available</Text>
                      </View>
                   )}
                </View>
            </View>
